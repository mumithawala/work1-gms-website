    ...
    jQuery.fn.beforeAfter = function (options) {
        ...
        function beforeAfter(that) {
            ...
            if (jQuery(that).children().length == 2) {
                ...
                div2.setAttribute('style', 'position:absolute !important;width:100% !important;height:' + settings.position + '% !important;top:0px !important;left:0px !important;overflow:hidden !important;' + slctNone);
                jQuery(secondImg).clone({
                    withDataAndEvents: true
                }).appendTo(div2);

                var left_arrow = document.createElement("I");
                left_arrow.setAttribute('style', 'border:solid black;border-width:2px 2px 0px 0px;display:inline-block;padding:8px;transform:rotate(135deg);-webkit-transform:rotate(135deg);');

                var right_arrow = document.createElement("I");
                right_arrow.setAttribute('style', 'border:solid black;border-width:0px 0px 2px 2px; margin-left:-18px; margin-bottom:8px;display:inline-block;padding:8px;transform:rotate(135deg);-webkit-transform:rotate(135deg);');

                var separator_bullet = document.createElement("DIV");
                separator_bullet.setAttribute('style', 'position:absolute;width:60px;height:60px;line-height:70px;padding:4xp;text-align:center;top:50%;left:50%;transform:translate(-50%,-50%);border-radius:50%;background-color:#fff;');
                ...
                separator.setAttribute('style', 'position:absolute !important;width:100% !important;height:2px !important;top:' + settings.position + '% !important;left:0px !important;overflow:visible !important;transform:translateY(-50%) !important;background-color: #fff;cursor:n-resize;opacity:' + settings.opacity + ';z-index:' + fZIndexInt + 2 + ";" + slctNone);
                ...
                if (settings.movable) {
                    ...
                    if (settings.clickMove) {
                        ...
                        jQuery(div1).on('click', function () {
                            ...
                            var oHeight = jQuery(that).height();
                            var oTop = jQuery(separator).css('top');
                            var oTopAdd = parseInt(oTop) + Math.min((oHeight * 0.05), 5);
                            var oPercentage = ((oTopAdd / oHeight) * 100).toFixed(2);
                            var correctTop = oPercentage > 100 ? 100 : (oPercentage < 0 ? 0 : oPercentage);
                            correctTop = correctTop + "%";
                            jQuery(div2).height(correctTop);
                            jQuery(separator).css('top', correctTop);
                            if (typeof settings.onMoving != "undefined" && oTop != jQuery(separator).css('top')) {
                                settings.onMoving();
                            }
                    ...
                    var mouseMove = function (e) {
                        ...
                        var oPageY = e.pageY || e.touches[0].clientY;
                        var oTop = jQuery(that).offset().top;
                        var oHeight = jQuery(that).height();
                        var oCurrentTop = jQuery(separator).css('top');
                        var oPos = (oPageY - oTop);
                        var oPercentage = ((oPos / oHeight) * 100).toFixed(2);
                        var correctTop = oPercentage > 100 ? 100 : (oPercentage < 0 ? 0 : oPercentage);
                        correctTop = correctTop + "%";
                        jQuery(div2).height(correctTop);
                        jQuery(separator).css('top', correctTop);

                        if (typeof settings.onMoving != "undefined" && oCurrentTop != jQuery(separator).css('top')) {
                            settings.onMoving();
                        }
